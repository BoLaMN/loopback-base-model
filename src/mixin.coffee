module.exports = (Model, options) ->
  ...
  { defaultMethods } = Model.definition.settings
  ...
  Model.once 'attached', (server) ->
    ...
    if not defaultMethods
      defaultMethods = 'all'

    if typeof defaultMethods is 'string'
      selectedMethods = switch defaultMethods
        when 'all' then methodKeys
        when 'none' then []
        when 'extended' then methodKeys.concat extendedKeys
    else if Array.isArray defaultMethods
      selectedMethods = defaultMethods
        .filter (defaultMethod) ->
